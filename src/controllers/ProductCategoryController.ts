...
const productCategory = {
  createProductCategory: async (req: Request, res: Response) => {
    try {
      ...
      const validateSiteRequest =
        productCategoryValidator.createProductCategoryValidator.validate(
          req.body,
        );
      ...
      const createProductCategory =
        await productCategoryService.productCategory(req.body);
  ...
  getProductCategory: async (req: Request, res: Response) => {
    try {
      ...
      const productCategory = await productCategoryService.getProductCategory(
        productCategory_id,
      );
      res.json({
  ...
  updateProductCategory: async (req: Request, res: Response) => {
    try {
      ...
      const { productCategory_id } = req.params;
      const validateSiteRequest =
        productCategoryValidator.updateProductCategoryValidator.validate(
          req.body,
        );
      if (validateSiteRequest.error) {
        return res.status(422).send("UNPROCESSABLE_ENTITY");
  ...
  deleteProductCategory: async (req: Request, res: Response) => {
    try {
      ...
      const deleteProductCategory =
        await productCategoryService.deleteProductCategory(productCategory_id);

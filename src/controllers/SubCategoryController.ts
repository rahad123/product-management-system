...
const subCategory = {
  createSubCategory: async (req: Request, res: Response) => {
    try {
      ...
      const validateSiteRequest =
        subCategoryValidator.createSubCategoryValidator.validate(req.body);
      ...
      const createCategory = await subCategoryService.createSubCategory(
        req.body,
      );
  ...
  updateSubCategory: async (req: Request, res: Response) => {
    try {
      ...
      const { subCategory_id } = req.params;
      const validateSiteRequest =
        subCategoryValidator.updateSubCategoryValidator.validate(req.body);
      if (validateSiteRequest.error) {
        return res.status(422).send("UNPROCESSABLE_ENTITY");
      }
  ...
  deleteSubCategory: async (req: Request, res: Response) => {
    try {
      const { subCategory_id } = req.params;
      const deleteSubCategory = await subCategoryService.deleteSubCategory(
        subCategory_id,
      );
